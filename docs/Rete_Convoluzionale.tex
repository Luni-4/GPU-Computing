...
\begin{frame}{Reti Neurali}
    ...
    \begin{block}{Definizione} 
        \large Un livello è di tipo \emph{Fully-Connected} quando i neuroni di due livelli adiacenti sono completamente connessi tra loro ed i neuroni che formano un livello non condividono nessuna connessione 
    \end{block}\pause
    
    \begin{itemize} [<+->]
        \setlength\itemsep{2em}
        \item \large I pesi associati ai neuroni di un livello sono salvati in matrici
        \item \large Le righe della matrice corrispondono ai neuroni del livello mentre le colonne ai pesi di ciascun neurone
        \item \large Le reti neurali sono organizzate con una struttura a livelli perché risulta più facile ed efficiente fare operazioni matriciali
    \end{itemize}
\end{frame}
