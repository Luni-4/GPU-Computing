...
namespace KernelStreamsBis {
	...
	void actReluK(dim3 b, dim3 t, cudaStream_t stream, double *output, double *temp, const int &nodes);
	//void derivActReluK(dim3 b, dim3 t, double *error, double *temp, const int &nodes);

	void actSigmoidK(dim3 b, dim3 t, cudaStream_t stream, double *output, const int &nodes);
	//void derivActSigmoidK(dim3 b, dim3 t, const double *output, double *error, const int &nodes);

	void actTanhK(dim3 b, dim3 t, cudaStream_t stream, double *output, const int &nodes);
...
/*METODI CONVOLUZIONALE*/
	void createSubmatrixBisK(dim3 b, dim3 t, cudaStream_t stream, double * sub, const double * prevOutput, const int prevLayerWidth, const int filterWidth, const int stride, const int uniqueNodes);
