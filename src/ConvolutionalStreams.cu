...
void ConvolutionalStreams::forward_propagation_streams(const double * prevOutput, cudaStream_t stream, int currentStream) {
	...
	for (int i = 0; i < _depth; i++) {
		for (int j = 0; j < _prevLayerDepth; j++) {
			//da correggere per depth > 1
			(j == 0) ? beta = 0.0 : beta = 1.0;
			CHECK_CUBLAS(cublasDgemv(handle, CUBLAS_OP_T, _filterDim, (_uniqueNodes / _nStreams), &alpha, subForward + (j * _uniqueNodes * _filterDim) + subForwardPlus, _filterDim, weightRot + (i * _filterDim * _prevLayerDepth) + (j * _filterDim), 1, &beta, output + (i * _uniqueNodes) + outputPlus, 1));
...
void ConvolutionalStreams::calcError() {
...
void ConvolutionalStreams::back_propagation(const double *prevOutput, double *prevError, const double &learningRate, const bool notFirst) {
...
void ConvolutionalStreams::updateWeights(const double *prevOutput, const double &learningRate) {
	...
	for (int i = 0; i < _depth; i++) {
		for (int j = 0; j < _prevLayerDepth; j++) {
			CHECK_CUBLAS(cublasDgemv(handle, CUBLAS_OP_T, _uniqueNodes, _filterDim, &alpha, subBack + (j * _uniqueNodes * _filterDim), _uniqueNodes, errorRot + (i * _uniqueNodes), 1, &beta, tempWeight + ((i + j * _depth) * _filterDim), 1));
			//CHECK_CUBLAS(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, 1, _filterDim, _uniqueNodes, &backAlpha, errorRot + (i * _uniqueNodes), 1, subBack + (j * _filterDim), _uniqueNodes, &beta, tempWeight + ((i + j * _depth) * _filterDim), 1));
		}
	}
